...
export const ProfileTab: React.FC<ProfileTabProps> = ({
  ...
  const [bioInput, setBioInput] = useState(currentUser.bio || '✨ Exploring Nautical Streams on HeyLook');
  const [nauticalStatus, setNauticalStatus] = useState<'in_focus' | 'adrift' | 'last_anchored'>(
    currentUser.nautical_presence || 'in_focus'
  );
  ...
  useEffect(() => {
    ...
    setNauticalStatus(currentUser.nautical_presence || 'in_focus');
  }, [currentUser]);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between px-2">
        <h2 className="text-xl font-bold tracking-tight">Profile & Preferences</h2>
        ...
        <div className="space-y-4">
          {isEditing ? (
            <div className="space-y-3">
              ...
              {/* Status Selector */}
              <div>
                <label className="text-xs font-semibold text-slate-400 mb-1 block">Nautical Status:</label>
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-xl font-bold text-slate-100">{currentUser.full_name}</h3>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-around text-center">
            <div>
              <div className="text-lg font-bold text-slate-100">{dbPostsCount}</div>
              <div className="text-xs text-slate-400 font-medium">Published Posts</div>
            </div>
            <div>
              <div className="text-lg font-bold text-slate-100">{dbReelsCount}</div>
            ...
            <div>
              ...
              <div className="text-xs text-slate-400 font-medium">Presence Status</div>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <span className="text-xs text-slate-400 capitalize">{isDark ? 'Dark Mode' : 'Light Mode'}</span>
        </button>
